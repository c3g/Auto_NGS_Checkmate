...
is.complete <- function(ind,all_samples){
  ...
  ifelse(all(map_lgl(sample_regexes,function(r) {!is_empty(grep(r,all_samples[grep(ind,all_samples)]))})),TRUE,FALSE)
...
data_print <- data2[final_match %in% c("Unexpected match", "Unexpected unmatch"),
                    .(Sample1,Sample2,Correlation, State = final_match)]
...
data_print <- data2[,.(Sample1, Sample2, Correlation, State = final_match)]
...
# **Suspicious cases network graph** 
...
if(n_mismatch>0) {
data_print <- data2[,.(Sample1, Case1=ind1, Sample2, Case2=ind2, Correlation, State = final_match)]
