...
output:
  html_document:
    ...
    df_print: paged
params:
  full_table: "/Users/spety/Documents/"
  prefix: "prefix"
---
...
#params <- list()
#params$full_table <- "."
ind_regex <- "MoHQ-[A-Za-z]+-[0-9]+-[0-9]+"
sample_regexes <- c("DN","DT","RT")
file="ncm_test_dir"
cor_thres <- "0.5"
out <- "."
prefix <- "test"
...
### add directory here
data2 <- readRDS(file.path(out,"Correlation_dt.rds"))
...
## 
is.complete <- function(vector_ind,all_samples){
 
  ifelse(all(map_lgl(sample_regexes,function(r) {!is_empty(grep(r,all_samples[grep(vector_ind,all_samples)]))})),TRUE,FALSE)
...
tableHTML(summary_input_df,headers="",border = 0,widths = c('200','60')) 
```
...
# **Concordance tables** 
## **New suspicious results**  
 

```{r extract new suspicious results}
data_print <- data2[final_match %in% c("Unexpected match", "Unexpected unmatch"),
                    .(Sample1, Sample2, Correlation, State = final_match,Run1,Run2)]

new <- read.csv(new,header=F)
new_samples <- new$sample_name
...
summary <- map(seq_along(all_ind),function(i) {
 ...
 x <- short_data[grep(all_ind[i],short_data$Sample1),]
...
write.csv(summary,file.path(out,paste0("summary_table_NGSCM_",prefix,".csv")),row.names = F)
...
#                        Score = unlist(Score))
#write.csv(as.data.frame(summary_table),paste0(params$full_table,"summary_table_NGSCM_",params$prefix,".csv"),row.names = F)
```

## **All results**  
```{r full results}

data_print <- data2[,.(Sample1, Sample2, Correlation, State = final_match,Run1,Run2)]

if(nrow(data_print)>20000){
  write.csv(data_print,file.path(out,paste0("AllComparisons_table_NGSCM_",prefix,".csv")))
